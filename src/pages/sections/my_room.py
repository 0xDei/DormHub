...
class MyRoom(Section):
    def __init__(self, resident_page):
        ...
        if self.resident_page.data["room_id"] != "N/A":
            ...
            if len(roommate_cards) == 0: roommate_cards.append(ft.Container(ft.Text("You have no roommates", color=ft.Colors.GREY_300), expand=True, alignment=ft.alignment.center))
            ...
            top_info = ft.Container(ft.Text("You currently don't have a room", color=ft.Colors.BLACK), expand=True, alignment=ft.alignment.center)
            room_info = ft.Container()
            room_mate = ft.Container()
